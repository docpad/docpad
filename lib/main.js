...
var DocPad = {
			...
			parseFile = function(fileFullPath,fileRelativePath,fileStat,next){
				...
				fileData = fs.readFile(fileFullPath,function(err,data){
					...
					if ( fileSplit.length === 3 && !fileSplit[0] ) {
						// Extract Parts
						fileHead = fileSplit[1].replace(/\t/g,'    ').replace(/\s+$/m,'')+'\n';
					...
					fileMeta.relativeBase = (relativeDirPath ? relativeDirPath+'/' : '')+path.basename(fileRelativePath,path.extname(fileRelativePath));
				...
				DocPad.DocumentModel.find({tags:{'$in':Document.tags}},function(err,relatedDocuments){
					...
					if ( relatedDocuments.length !== 0 ) {
						relatedDocuments.sort(function(a, b) { 
							...
							if ( Document.url === relatedDocument.url ) return;
	...
	skeleton: function(next){
		...
		path.exists(DocPad.options.srcPath,function(exists){
			if ( exists ) {
				...
				util.cpdir(DocPad.options.skeletonPath,DocPad.options.srcPath,function(){
	...
	server: function(next){
		...
		app.configure(function(){
			...
			app.use(express.errorHandler()); // enable error handling
			app.use(express.bodyParser()); // enable form handling
		...
		app.post(/^\/edit(.*)/, DocPad.editAction);
		...
		DocPad.DocumentModel.findOne({url:url},function(err,Document){
			...
			if ( Document ) {
				if ( req.body && req.body.content ) {
					...
					Document.save(function(err){
						if ( err ) throw err;
						fs.writeFile(Document.fullPath,Document.content,function(err){
							if ( err ) throw err;
							next(Document);
						});
					});
